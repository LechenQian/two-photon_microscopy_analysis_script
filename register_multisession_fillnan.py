...
for mouse in deg_d1+deg_d2+con_d1+con_d2:
    ...
    mat = np.zeros([alignments.shape[0],45])

    for ttype in getKeys(big_dict[mouse]['aligned_dff_multitrials']):
        if ttype == 'unpred_water':
            for i in range(6):
                for j in range(alignments.shape[0]):
            
                    if i>2:
                        try:
                    
                    
                            mat[j,:] = big_dict[mouse]['aligned_dff_average'][ttype][i-3][alignments[j, i], :]
                    
                        except:
                            pass
                big_dict[mouse]['registered_aligned_dff_average'][ttype].append(mat)
            ...
            for i in range(6):
                for j in range(alignments.shape[0]):
                    ...
                    try:
                        ...
                        mat[j,:] = big_dict[mouse]['aligned_dff_average'][ttype][i][alignments[j, i], :]
...
filename = "from_mice_pickle_and_hdf5_loaded_processed_2021-11-03_registration_nan_filled_zero.pickle"
...
path = "D:/PhD/Microscope/Big_dict_for_imaged_mice/all_pickled"
filename = "partial_to_mac_2021-11-03_avg_trace_registered_nan_filled_added"
